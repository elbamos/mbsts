...
params:
  ...
  p: 3                            
  q: 3
  ar: 1                           # Number of periods for trend model
  s: !r c(2)              # Seasonality periods
  corr_prior: 4
...
for (t in 1:(periods_to_generate + seasoning)) {
...
```{r sample}
samples <- sampling(model, data=stan_data, chains=2, iter=1000, init="0", cores=2
...
price_forecast %>%
  ...
  theme_minimal() +
  scale_y_continuous(limits=c(0, 5500), oob=function(x, limits) case_when(
    x > limits[2] ~ limits[2], 
    x < limits[1] ~ limits[1], 
    TRUE ~ x
