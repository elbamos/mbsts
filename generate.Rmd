...
tau %>% data.frame() %>%
  ...
  group_by(series) %>%
  arrange(period) %>%
  mutate(seasonality = exp(cumsum(seasonality))) %>% 
  ggplot(aes(x = period, y=seasonality, color=series)) + 
  geom_line(size=0.5, alpha=0.4) +
  geom_vline(data=period_vline, aes(xintercept=xintercept), color="red") +
  theme_minimal() + 
  ggtitle("Generated Seasonality (Cumulative)")
```
## Cyclicality
...
omega %>% data.frame() %>%
  ...
  group_by(series) %>%
  arrange(period) %>%
  mutate(cyclicality = exp(cumsum(cyclicality))) %>% 
  ggplot(aes(x = period, y=cyclicality, color=series)) + 
  geom_line(size=0.5, alpha=0.4) +
  geom_vline(data=period_vline, aes(xintercept=xintercept), color="red") +
  theme_minimal() + 
  ggtitle("Generated Cyclicality (Cumulative)")
...
epsilon %>% data.frame() %>%
  ...
  group_by(series) %>%
  arrange(period) %>%
  mutate(innovation = exp(cumsum(innovation))) %>% 
  ggplot(aes(x = period, y=innovation, color=series)) + 
  geom_vline(data=period_vline, aes(xintercept=xintercept), color="red") +
  geom_line(size=0.5, alpha=0.4) +
  theme_minimal() + 
  ggtitle("Generated Innovations (Cumulative)")
```
