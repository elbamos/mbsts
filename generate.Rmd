...
```{r sample}
stan_data <- exact_stan_data #sampled_stan_data
samples <- sampling(model, data=stan_data, chains=4, iter=2000, cores=3)
samples %<>% recover_types(prices_gathered %>% dplyr::mutate(id = 1:n()))
# samples %<>% recover_types(sampled_prices_for_stan)
```
...
true_prices <- prices %>% 
  ...
  mutate_all(cumsum) %>%
  mutate_all(exp) %>% 
  gather(key="series", value="y_hat", -period) %>% 
  mutate(.chain = "gen")
...
periodic_price_hat %>%
  ...
  bind_rows(true_prices, mean_periodic_price_hat) %>% 
...
coefficients_hat <- samples %>% gather_draws(beta_trend[feature, series], beta_xi[feature, series], 
                                             beta_p[feature, series], beta_q[feature, series])

coefficients_hat %>%
  ggplot(aes(x = .value)) +
  geom_histogram(bins=60) +
  facet_grid(series ~ .variable + feature, scales="free") +
  theme_minimal() +
...
volatilities_hat <- samples %>% gather_draws(theta_trend[series], alpha_trend[series], theta_cycle[series], theta_season[series], omega_garch[series])

volatilities_hat %>% 
  ggplot(aes(x = .value)) +
  geom_histogram(bins=60) +
  facet_grid(.variable ~ series) +
  theme_minimal() +
