...
params:
  ...
  s: !r c(5, 20, 60)              # Seasonality periods
...
```{r trends}
theta_trend <- abs(rcauchy(params$S, 0, 1e-4)) / params$period_scale
omega_trend <- rlkjcorr(1, params$S)
alpha_trend <- rnorm(params$S, 0, .001) / params$period_scale
...
for (t in 1:(periods_to_generate + seasoning)) {
  if (t < params$s[3]) tau <- c(tau, list(w[t, ]))
  else {
    prior_tau <- do.call(rbind, tau[(t - params$s[3] + 1):(t-1)])
