...
params:
  ...
  from_date: !r as.Date("2018-06-06")
  ar: 2
  p: 2
  q: 2
  s: !r c(5, 20)
...
```{r prepare_data}
vix <- data %>%
  dplyr::filter(series == "X.VIX") %>%
  arrange(period) %>% 
  dplyr::filter(period < max(data$period) - leave_out)

spy <- data %>%
  dplyr::filter(series == "SPY") %>%
  arrange(period) %>% 
  dplyr::filter(period < max(data$period) - leave_out)
...
```{r model}
model <- stan_model("./mbsts_fin.stan")
```

```{r sample}
samples <- sampling(model, data=stan_data, chains=3, iter=2000, cores=4,
