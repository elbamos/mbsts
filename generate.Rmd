...
samples <- sampling(model, data=stan_data, chains=3, iter=2000, init=init_f, cores=4, save_warmup=FALSE
...
bind_rows(prices_gathered %>% 
  ...
  scale_fill_brewer("Confidence", palette="Blues") +
  scale_y_continuous(limits=c(0, 3500), oob=function(x, limits) case_when(
    x > limits[2] ~ limits[2], 
    x < limits[1] ~ limits[1], 
    TRUE ~ x
  )) +
