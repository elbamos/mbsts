...
model {
  ...
  for (t in 1:(N_periods-1)) {
    w_t[t] ~ normal(zero_vector, theta_season_hs);
...
generated quantities {
  ...
  for (t in 1:periods_to_predict) {
    ...
    kappa_star_hat[t] = multi_normal_rng(zero_vector', diag_matrix(theta_cycle_hs))';
    w_t_hat[t] = multi_normal_rng(zero_vector', diag_matrix(theta_season_hs))';
  }
