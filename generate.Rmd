...
params:
  ...
  s: !r c(5, 20)              # Seasonality periods
...
```{r sample}
samples <- sampling(model, data=stan_data, chains=2, iter=2000, init="0", cores=2) %>% recover_types(price_set)
```
...
bind_rows(prices_gathered %>% 
  ...
  geom_lineribbon(alpha=0.2, size=0.5, data=price_forecast %>% 
                  ...
                  group_by(period, series, .chain) %>% 
                  median_qi(y_hat, .width=c(0.2, 0.3, 0.66))
                  ) +
