...
set.seed(19746)
pad <- 100
periods_to_generate <- params$P + params$periods_to_predict + pad
...
delta <- marima.sim(kvar=5,
                 ...
                 nstart=500, 
                 nsim=params$P + params$periods_to_predict)
...
for (t in 1:periods_to_generate) {
  if (t < params$s[1]) tau <- c(tau, list(w[t, ]))
  else {
...
garchout <- eccc.sim(params$P + params$periods_to_predict, 
                     theta_epsilon, A=diag(beta_p), B=diag(beta_q), 
...
shrinkage_hat %>% 
  ...
  geom_histogram(bins=60) +
  geom_vline(data=data.frame(xintercept=0), aes(xintercept=xintercept), color="red", size=0.2) +
  facet_grid(.variable + feature ~ series, scales ="free") +
  theme_minimal()
```
...
volatilities_hat <- samples %>% gather_draws(theta_ar[series], alpha_ar[series], theta_cycle_hs[series], theta_season_hs[seasonality][series], omega_garch[series]) %>%
  ungroup() %>% 
  mutate(
    component = case_when(
      .variable %in% c("theta_season", "theta_season_hs") ~ paste("theta_season", seasonality, sep="_"),
      TRUE ~ .variable
    )
  )
